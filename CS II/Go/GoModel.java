...
public class GoModel {
	...
	public boolean canReach(Location point, Color color, Color targetColor, Set<Location> visited) {
		//wrote this one to check that point is still on the board
		if(!isOnBoard(point)) {
			return false;
		}
	...
	//added this. just in case (i dont know if its required)
	 ...
	 */
	//if it is occupied by or surrounded by the player, that player "owns" it
	...
	//saves back to a previous model
	public void undo() {
		if (!previousStates.isEmpty()) {
			GoModel previous = previousStates.pop();
