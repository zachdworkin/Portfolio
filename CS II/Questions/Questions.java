...
public class Questions {
	...
	public void drawSubtree(Node node, double y, double yDecrement, double left, double right) {
		double mid = (left + right) / 2.0;//middle
		if (!node.isLeaf()) {
			StdDraw.line(mid, y, (left + mid) / 2.0, y - yDecrement);//go down and left
			StdDraw.line(mid, y, (right + mid) / 2.0, y - yDecrement);//go down and right
			drawSubtree(node.getLeft(), y - yDecrement, yDecrement, left, mid);//get the left side and continue
			drawSubtree(node.getRight(), y - yDecrement, yDecrement, mid, right);//get the right side and continue
		}
		if (node == model.getCurrentNode()) {//draw a red circle for where we are
			StdDraw.setPenColor(StdDraw.RED);
		}
		StdDraw.filledCircle(mid, y, CIRCLE_RADIUS);//draw a black circle for all other nodes
